...
if(((typeOF _curTarget) in listaCasas) && playerSide  isEqualTo  west) Then {
	...
	if!(_curTarget in life_vehicles) then {
		...
		if ((typeof _curTarget) in apartamentos) then {
			_ButText = "Comprar Apartamento";
			_ButTopTip = "Comprar un apartamento o piso en este edificio";
			_ButAction = " [life_pInact_curTarget] spawn life_fnc_buyApartamento ";
		} else {
		...
		_ButEnable = true;
		if(!isNil {_curTarget getVariable "house_owner"} && !((typeof _curTarget) in apartamentos)) then { _ButEnable = false };
		_MakeButon = [_MakeButon,_ButText,_ButTopTip,_ButAction,_ButIcon,_ButEnable] call bk_fnc_ActionMenuAdd;
		
		_ButText = "Ganzuar";
		_ButTopTip = "Ganzuar esta casa ejem he perdido las llaves";
		_ButAction = " [life_pInact_curTarget] spawn life_fnc_lockpick ";
		...
		if((typeOf _curTarget) in listaGarajes) then {
			...
			if ((typeof _curTarget) in apartamentos) then {
				_ButText = "Vender Apartamento";
				_ButTopTip = "Vender este cuasi penthouse";
				_ButAction = " [life_pInact_curTarget] spawn life_fnc_sellHouse ";
				...
				_ButText = "Ganzuar Apartamento Vecino";
				_ButTopTip = "Ganzuar este cuasi penthouse";
				_ButAction = "  [life_pInact_curTarget] spawn life_fnc_lockpick  ";
				_ButIcon = "MenuIcoCas_CasaForzar";
				if(["pop_ganzua"] call ica_fnc_tengo > 0) then { // Hay que poner la condicion tb de la puerta
					_ButEnable = true;
			...
			if ((typeof _curTarget) isEqualTo "land_POP_Mansion_4" && life_gangCasa isEqualTo []) then {
				_ButText = "Convertir en Casa de Mafia";
				_ButTopTip = "Vuelve esta casa accesible a toda la mafia";
				_ButAction = " [life_pInact_curTarget] spawn life_fnc_gangCreoCasa";
				_ButIcon = "MenuIcoCas_CasaVender";
				_ButEnable = true;
				if(((_curTarget getVariable "house_owner") select 0) != (getPlayerUID player) OR life_gangrank != 5) then { _ButEnable = false; };
				//if !(life_gangCasa isEqualTo []) then { _ButEnable = false; };
				_MakeButon = [_MakeButon,_ButText,_ButTopTip,_ButAction,_ButIcon,_ButEnable] call bk_fnc_ActionMenuAdd;				
			};
