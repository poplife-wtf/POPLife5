/*
	...
	Author: Steven Campbell //Edit Rosen
*/
...
{{_x animate ["switchred",0];} foreach bs;} remoteExec ["bis_fnc_spawn",-2];
...
_lightLoop = {
	...
	if(counter  isEqualTo  0) exitWith {		
		{{_x animate ["switchgreen",0];}foreach active;} remoteExec ["bis_fnc_spawn",-2];		
		{{_x animate ["switchred",1];}foreach active;} remoteExec ["bis_fnc_spawn",-2];		
		counter = 3;
	};
	...
	if(counter  isEqualTo  18) exitWith {
		{{_x animate ["switchgreen",1];}foreach active;} remoteExec ["bis_fnc_spawn",-2];		
		{{_x animate ["switchyellow",0];}foreach active;} remoteExec ["bis_fnc_spawn",-2];
		counter = 21;
	};
	...
	if(counter  isEqualTo  24) exitWith {
		{{_x animate ["switchyellow",1];}foreach active;} remoteExec ["bis_fnc_spawn",-2];		
		{{_x animate ["switchred",0];}foreach active;} remoteExec ["bis_fnc_spawn",-2];	
		counter = 27;
	};
	...
	if(isServer && {life_loopExit}) exitWith {
		...
		lightSwitch = nil;
		counter = nil;
		[] remoteExecCall ["TON_fnc_trafficLights",player];
	};
...
[_lightLoop, 4, []] call CBA_fnc_addPerFrameHandler;