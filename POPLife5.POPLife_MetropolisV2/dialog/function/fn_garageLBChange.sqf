...
_retrievePrice = if(_retrievePrice == -1) then {100} else {(__GETC__(life_garage_prices) select _retrievePrice) select 1;};
_sellPrice = if(_sellPrice == -1) then {100} else {(__GETC__(life_garage_sell) select _sellPrice) select 1;};
