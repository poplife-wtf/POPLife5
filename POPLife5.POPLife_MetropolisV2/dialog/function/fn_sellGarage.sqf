...
if (!(_sellPrice isEqualType 0) || _sellPrice < 1 || _sellPrice > 500) then {_sellPrice = 500;};
...
pop_atmdin = pop_atmdin + _sellPrice;
[1] call SOCK_fnc_updatePartial;

if (LIFE_SETTINGS(getNumber,"player_advancedLog") isEqualTo 1) then {
    if (LIFE_SETTINGS(getNumber,"battlEye_friendlyLogging") isEqualTo 1) then {
        advanced_log = format [localize "STR_DL_AL_soldVehicle_BEF",_vehicleLife,[_sellPrice] call life_fnc_numberText,[pop_atmdin] call life_fnc_numberText,[pop_din] call life_fnc_numberText];
    } else {
        advanced_log = format [localize "STR_DL_AL_soldVehicle",profileName,(getPlayerUID player),_vehicleLife,[_sellPrice] call life_fnc_numberText,[pop_atmdin] call life_fnc_numberText,[pop_din] call life_fnc_numberText];
    };
