...
ctrlSetText[1003,format["Tu Agrupación: %1",(_gang select 0)]];
...
{	
	...
	{		
		...
		{			
			_members lbAdd format["%1 [Lider]",name (_units select _i)];		
		};				
		
		case ((getPlayerUID (_units select _i)) == _ownerID) :		
		{			
			_members lbAdd format["%1 [Fundador]", name (_units select _i)];		
		};				
