...
racing_lastCP = time;
racing_timeOut = 60; //Tiempo maximo entre punto y punto.
	...
	while {inRace} do{
...

		{
			if(waypointIndex < count currentTrackOrbs) then {
				if(_x == _cur) then {
					_found = true;
					_x setObjectTextureGlobal [0, "#(argb,8,8,3)color(0,0,1,1)"];
				} else {
					if(!_found) then {
				...
				_x setObjectTextureGlobal [0, "#(argb,8,8,3)color(0,1,0.2,1)"];
			};
