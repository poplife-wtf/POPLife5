...
if !(isNull objectParent player) exitWith {
	...
	[[objectParent player,0],"life_fnc_lockVehicle",objectParent player,false] call life_fnc_MP;
	if(!(objectParent player in life_vehicles)) then
	{
		life_vehicles set[count life_vehicles,objectParent player];
	};
...
if(!isNull _curTarget) then {
	...
	_nearVehicles = nearestObjects[getPos player,["Car","Air","Ship","Truck","Motorcycle"],10];
