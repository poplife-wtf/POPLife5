...
if (count(group player getVariable ["gang_members",15]) == (group player getVariable ["gang_maxMembers",8])) exitWith {hint localize "STR_GNOTF_MaxSlot"};
...
if (_action) then {
	...
	if (count _members > _maxMembers) exitWith {hint "Ya tienes el numero maximo de integrantes en tu Mafia."};
