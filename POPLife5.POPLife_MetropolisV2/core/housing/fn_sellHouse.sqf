...
// FuRixX
_vendiendo = player getVariable ["FuRixX_Tiempo_CasaVender", time - 35];
_resta = (time - _vendiendo) / 60;


if (_resta < 0.5) exitWith {
    hint "Solo puedes usar esta acción cada 30 segundos";
};
...
player setVariable ["FuRixX_Tiempo_CasaVender", time];
...
if (_action) then {
    ...
    if (LIFE_SETTINGS(getNumber,"player_advancedLog") isEqualTo 1) then {
        if (LIFE_SETTINGS(getNumber,"battlEye_friendlyLogging") isEqualTo 1) then {
            advanced_log = format [localize "STR_DL_AL_soldHouse_BEF",(round((_houseCfg select 0)/2)),[pop_atmdin] call life_fnc_numberText];
        } else {
            advanced_log = format [localize "STR_DL_AL_soldHouse",profileName,(getPlayerUID player),(round((_houseCfg select 0)/2)),[pop_atmdin] call life_fnc_numberText];
            };
