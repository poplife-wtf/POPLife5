...
if (_param isEqualTo "usar") exitWith {
	...
	//sleep 2;
	//[silla_electrica] remoteExec ["life_fnc_tazeSound", -2];
	["global", "sillaElectrica", _victima] call ica_fnc_SFX;
	sleep 14;
	_victima setdamage 1;
	sleep 1;
	detach _victima;
	_nombrevictima = name _victima;
	[10, "El dpto. penitenciario de Plata o Plomo informa", [1,0,0], 1.5, format["Se ha impartido justicia, %1 acaba de ser ejecutado en la silla electrica.",_nombrevictima], [1,1,1], 1] remoteExec ["ica_fnc_anuncio", -2];
};
