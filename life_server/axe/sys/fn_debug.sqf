...
fnc_admin_ccc = compileFinal 'if ([''ica_fnc_'',_this] call ica_fnc_inString)then{''Q intentas?''}else{if(!isNil {call compile _this})then{call compile _this}else{''ANY''}}';
fnc_createctrl = {
...
true
				...
				admin_global = {compile _this remoteExec ["spawn",0]};
				admin_server = {compile _this remoteExec ["spawn",2]};
				admin_globalmserver = {compile _this remoteExec ["spawn",-2]};								
...
_ctrl ctrlCommit 0;
_ctrl ctrlSetEventHandler['ButtonClick','(call fnc_getMainText) call globalmserver;systemChat ''<PoPLife v.5> Codigo ejecutado en todos menos el server'';true'];
...
_ctrl ctrlCommit 0;
_ctrl ctrlSetEventHandler['ButtonClick','hint "esta mierda paq"'];
};
