...
Falta 
	...
	["init"] spawn ica_fnc_centralica; // para inicializar el loop de posiciones en el mapa
...
if (_param isEqualTo "resetP") exitWith {
...
if (_param isEqualTo "resetM") exitWith {
...
arrCentralica_P = HW getVariable "centralica_p";
arrCentralica_M = HW getVariable "centralica_m";

	if (isNil "arrCentralica_P") then {
		["resetP"] call ica_fnc_centralica;
		sleep 1;
	};

	if (isNil "arrCentralica_M") then {
		["resetM"] call ica_fnc_centralica;
		sleep 1;
	};
...
if (_param isEqualTo "menu") exitWith {
	...
	createDialog "centralica";

	_ui = uiNamespace getVariable "centralica";
	centralica_selec = nil;
	...
	if (playerside isEqualTo WEST) then {
		_boton3 ButtonSetAction '["resetP"] call ica_fnc_centralica';
	} else {
		_boton3 ButtonSetAction '["resetM"] call ica_fnc_centralica';
	};
	...
	// Config botones check y demás
	{
		...
		_x ctrlAddEventHandler ["ButtonClick", {
			["selec", _this select 0] call ica_fnc_centralica
		}]; // devuelve de l1b a l30b
	...
	_check ctrlAddEventHandler ["CheckedChanged", {
		...
		_ui = uiNamespace getVariable "centralica";
		_mensaje = _ui displayCtrl 12000;
		
		_mensaje ctrlSetFade (_this select 1);
		_mensaje ctrlCommit 0.5;
	...
	// Selecciono el array de mi bando
	_arrBando = if ((side player) == WEST) then {arrCentralica_P} else {arrCentralica_M};

	_msgDia ctrlSetStructuredText parseText format ["<t size='1.1' align='center'>%1</t>", _arrBando select 0]; // mensaje del día
	_entrada2 ctrlSetText (_arrBando select 0);
	...
	for "_i" from 0 to 29 do {
		...
		// Nombre
		(_textos select _i) ctrlSetStructuredText parseText format ["<t size='1.1' align='center'>%1</t>", (_arrBando select (_i + 1) select 0)];
		
		
		// Color
		_color = switch (_arrBando select (_i + 1) select 1) do { // convierto string de color a RGBA
		...
		} forEach (_arrBando select (_i + 1) select 2); // personas
	};
	...
	_boton1 ctrlAddEventHandler ["ButtonClick", {
		...
		if (isNil "centralica_selec") exitWith {hint "No has seleccionado un grupo"};
		
		
		// Actualizo texto
		_ctrl = call compile (format ["_txt%1", centralica_selec]);
		
		_arrBando = if ((side player) == WEST) then {arrCentralica_P} else {arrCentralica_M};		
		
		_struct = parseText format ["<t align='center'>%1</t>", ctrlText _entrada];
		(_arrBando select centralica_selec) set [0, ctrlText _entrada]; // lo meto en el array
		_ctrl ctrlSetStructuredText _struct;
		if ((side player) == WEST) then {HW setVariable ["centralica_p",_arrBando,true]} else {HW setVariable ["centralica_m",_arrBando,true]};
		
		// Actualizo color
		_idxLista = lbCurSel _selCol;
		_color = (_selCol lbData _idxLista);
		
		(_arrBando select centralica_selec) set [1, _color];
		if ((side player) == WEST) then {HW setVariable ["centralica_p",_arrBando,true]} else {HW setVariable ["centralica_m",_arrBando,true]};
		
		["act"] call ica_fnc_centralica;
	...
	_boton2 ctrlAddEventHandler ["ButtonClick", {
		...
		_arrBando set [0, ctrlText _entrada]; // lo pongo en el array
	...
	_nivelx = switch (playerside) do {
	case WEST : {life_coplevel};
	case INDEPENDENT : {life_medicLevel};
	default {life_coplevel};
	};
	if ((call _nivelx ) < 2) then {
	...
	// centralica_selec = 1; // empieza desde el 1
	// ["selec", _bot1] call ica_fnc_centralica; // selecciono el primer grupo manualmente
	["act"] call ica_fnc_centralica; // actualizo todo
...
private [
	"_msgDia", "_check", "_listaSG", "_entrada", "_entrada2", "_selCol", "_boton", "_boton1", "_boton2",
	"_txt1", "_lis1", "_bot1",
	"_txt2", "_lis2", "_bot2",
	"_txt3", "_lis3", "_bot3",
	"_txt4", "_lis4", "_bot4",
...
disableSerialization;
if (!isNil {uiNamespace getVariable "centralica"}) then {
...
if (_param isEqualTo "selec") exitWith {
	...
	if (isNil "centralica_selec") exitWith {
		centralica_selec = _idx;
	...
	if (centralica_selec == _idx) then { // he vuelto a hacer click en la misma
		...
		(call compile (format ["_txt%1", centralica_selec])) ctrlSetBackgroundColor [0,0,0,1];
		...
		centralica_selec = _idx; // actualizo variable de lo que tengo seleccionado
	};
	...
	_arrBando = if ((side player) == WEST) then {arrCentralica_P} else {arrCentralica_M};
	_colorActual = (_arrBando select _idx select 1);
...
if (_param isEqualTo "act") exitWith {
	...
	if (!isNil "centralica_selec") then {
		_nomG = _arrBando select centralica_selec select 0;
		_personas = _arrBando select centralica_selec select 2;
		
		
		// Actualizo info botón
		if (player in _personas) then {
			_boton ctrlSetText format ["DEJAR %1", _nomG];
			_boton ButtonSetAction '["pulso", false] call ica_fnc_centralica';	
		} else {
			_boton ctrlSetText format ["UNIRSE A %1", _nomG];
			_boton ButtonSetAction '["pulso", true] call ica_fnc_centralica';
		};
	...
	_arrTodos = [];
	for "_i" from 1 to (count _arrBando)-1 do { // Saco lista de toda la peña metida en grupo
		_arrTodos append (_arrBando select _i select 2);
	};
	
	
	{
		if ((side _x) isEqualTo (side player)) then { // si es de mi SIDE
			if !(_x in _arrTodos) then { // si está sin grupo (no grupo de arma, en el array de la centralica)
...
if (_param isEqualTo "pulso") exitWith {
	...
	if (_modo) then { // me uno
		...
		if (!isNil "centralica_enGrupo") then { // ya estoy en grupo y me quiero unir a otro
			(_arrBando select centralica_enGrupo select 2) deleteAt ((_arrBando select centralica_enGrupo select 2) find player); // me salgo de mi grupo
			if ((side player) == WEST) then {HW setVariable ["centralica_p",_arrBando,true]} else {HW setVariable ["centralica_m",_arrBando,true]};
		};
		
		if (!isNil "centralica_selec") then {
			centralica_enGrupo = centralica_selec;
			(_arrBando select centralica_selec select 2) pushBack player;
			if ((side player) == WEST) then {HW setVariable ["centralica_p",_arrBando,true]} else {HW setVariable ["centralica_m",_arrBando,true]};
		};
		...
		centralica_enGrupo = nil;
		
		if (isNil "centralica_selec") exitWith {hint "Primero tienes que seleccionar el grupo que quieres dejar."};
		(_arrBando select centralica_selec select 2) deleteAt ((_arrBando select centralica_selec select 2) find player);
...
if (_param isEqualTo "init") exitWith {
	...
	0 spawn {
		...
		while {!isNil "ci_mostrandoMapa"} do {
			...
			for "_i" from 1 to (count _arrBando)-1 do {
				_grupo = _arrBando select _i; // arrBando tiene elementos como este --> ["NombreGrupo", "colorRed", [tio1, tio2...]]
