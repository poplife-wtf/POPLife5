...
switch (_switch) do {
	case 0: {
		if(!(count all_ah_items == 0)) then {
			_handle = {_id = _x select 0;_time = _x select 8;_time = round(_time - 1); if (_time < 1 && _x select 7 == 0) then {_status = 1} else {_status = _x select 7}; _toDel = round(_time + 48); if (_toDel < 1) then {[1,_id] spawn TON_fnc_vAH_update} else {[2,_id,_time,_status] spawn TON_fnc_vAH_update};} forEach all_ah_items;
	...
	case 1: {
		_query = format["DeleteInfo:%1",_id];
		[_query,1] call DB_fnc_asyncCall;
		diag_log format["Vendetta Auction House: ITEM REMOVED: %1",_query];
		[] spawn TON_fnc_vAH_init;
	};
	case 2: {
		_query = format["UpdateInfo:%1:%2:%3",_time,_status,_id];
		[_query,1] call DB_fnc_asyncCall;
	};
	case 3: {
		_query = format["UpdateInfor:%1",_id];
		[_query,1] call DB_fnc_asyncCall;
	...
	case 4: {
		...
		_query = format["InsertInfo:%1:%2:%3:%4:%5:%6:0:48",_id,_time,_items,_status,_seller,_playerName];
		[_query,1] call DB_fnc_asyncCall;
