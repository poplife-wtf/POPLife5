...
*/
private ["_query","_containers","_containerss","_houses","_granjas","_negocios"];
params [
    ["_uid","",[""]]
];
if (_uid isEqualTo "") exitWith {};

_query = format ["SELECT pid, pos, classname, inventory, gear, dir, id, apto FROM containers WHERE pid='%1' AND owned='1'",_uid];
...
_containerss = [];
{
	...
	//diag_log format ["--------- Contenedor de DB, apto = %1 -----------",_x select 7];
	
	if ((_x select 7) isEqualTo 0) then {
		_containerss = _house getVariable ["containers",[]];
	}else{
		_containerss = _house getVariable [format ["%1_containers",_x select 7],[]];
	};
	...
	if ((_x select 7) isEqualTo 0) then {
		...
		_house setVariable [format ["%1_containers",_x select 7],_containerss,true];
	};
...
_query = format ["SELECT pid, pos FROM granjas WHERE pid='%1' AND owned='1'",_uid];
_granjas = [_query,2,true] call DB_fnc_asyncCall;

{
    _pos = call compile format ["%1",_x select 1];
    _granjas = nearestObject [_pos, "House"];
    _granjas allowDamage false;
    _return pushBack [_x select 1];
} forEach _granjas;


_query = format ["SELECT pid, pos FROM negocios WHERE pid='%1' AND owned='1'",_uid];
_negocios = [_query,2,true] call DB_fnc_asyncCall;

{
    _pos = call compile format ["%1",_x select 1];
    _negocios = nearestObject [_pos, "House"];
    _negocios allowDamage false;
    _return pushBack [_x select 1];
} forEach _negocios;
