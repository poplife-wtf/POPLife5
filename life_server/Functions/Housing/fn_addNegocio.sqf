...
*/
private ["_housePos","_query","_negocio"];
params [
    ["_negocio",ObjNull],
	["_uid",""]
];

_housePos = getposATL _negocio;
_classname = typeof _negocio;
...
_query = format ["INSERT INTO negocios (pid, pos, owned, classname) VALUES('%1', '%2', '1', '%3')",_uid,_housePos,_classname];
if (EXTDB_SETTING(getNumber,"DebugMode") isEqualTo 1) then {
...
_query = format ["SELECT id FROM negocios WHERE pos='%1' AND pid='%2' AND owned='1' ",_housePos,_uid];
...
_negocio setVariable ["negocio_id",(_queryResult select 0),true]; //lulwat
