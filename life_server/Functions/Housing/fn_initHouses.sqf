...
for [{_x=0},{_x<=_count},{_x=_x+10}] do {
    _query = format ["SELECT houses.id, houses.pid, houses.pos, players.name, houses.garage, houses.apto FROM houses INNER JOIN players WHERE houses.owned='1' AND houses.pid = players.pid LIMIT %1,10",_x];
    ...
    if (count _queryResult isEqualTo 0) exitWith {};
    {
		...
		if ((_x select 5) isEqualTo 0) then {
			...
			_houseO = _house getVariable ["house_owner",[]];
			if (isNil "_houseO" OR !(typename _houseO isEqualTo "ARRAY")) then { _houseO = []};
			_house setVariable ["house_owner",(_houseO + [[_x select 1,_x select 3]]),true];
			_house setVariable [ format ["%1_owner",_x select 5],[_x select 1,_x select 3],true];
			_house setVariable [format ["%1_locked",_x select 5],true,true]; 
			_house setVariable [ format ["%1_id",_x select 5],_x select 0,true];
			_house setVariable [format ["bis_disabled_Door_%1",_x select 5],1,true];
