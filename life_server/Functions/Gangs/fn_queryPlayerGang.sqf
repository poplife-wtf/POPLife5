...
_query = format ["SELECT id, owner, name, maxmembers, bank, members, zona, casa FROM gangs WHERE active='1' AND members LIKE '%2%1%2'",_this,"%"];
...
if !(count _queryResult isEqualTo 0) then {
    ...
    _tmp = [_queryResult select 7] call DB_fnc_mresToArray;
    if (_tmp isEqualType "") then {_tmp = call compile format ["%1", _tmp];};
    _queryResult set[7, _tmp];	
};
