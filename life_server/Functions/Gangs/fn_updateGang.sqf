...
switch (_mode) do {
    ...
    case 5: {
        _posFinal = _group getVariable "gang_casa";
        _posFinal = [_posFinal] call DB_fnc_mresArray;
        _query = format ["UPDATE gangs SET casa='%1' WHERE id='%2'",_posFinal,_groupID];
    };	
	
    case 6: {
        _query = format ["UPDATE gangs SET zona='%1' WHERE id='%2'",(_group getVariable ["gang_zona",0]),_groupID];
    };	
};

[_group] remoteExec ["ica_fnc_updateMafia",units _group];
