...
private _query = format ["SELECT id, side, classname, type, pid, alive, active, plate, color, inventory, gear, fuel, damage, blacklist, colorido FROM vehicles WHERE id='%1' AND pid='%2'",_vid,_pid];
...
_vehicle allowDamage true;
_vehicle lock 2;
_colorido = call compile format ["%1",(_vInfo select 14)];
//Reskin the vehicle

diag_log format ["pintura coche db %1 typi %2",_colorido,typename _colorido];
if (_colorido isEqualTo "nada") then {}else{
_vehicle setObjectTextureGlobal [0,_colorido];
_vehicle setObjectTextureGlobal [1,_colorido];
_vehicle setObjectTextureGlobal [2,_colorido];
_vehicle setObjectTextureGlobal [3,_colorido];
_vehicle setObjectTextureGlobal [4,_colorido];
_vehicle setObjectTextureGlobal [5,_colorido];
};
_vehicle setVariable["vehicle_info_owners",[[_pid,_name,_vid]],true];
