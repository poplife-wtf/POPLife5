...
switch (_mode) do {
    ...
    case 22: {
		...
		//diag_log format ["recien recibido el _value en el server updatePartial 32 : exp  :   %1  type: %2",_value, typename _value];
		if (typename _value != "ARRAY") exitWith {
		//diag_log format ["err updatePartial 22 : exp  :   %1  type: %2",_value, typename _value];
		};
		if (count _value < 14) exitWith {
		//diag_log format ["err updatePartial 22 : exp  :   %1  type: %2",_value, typename _value];
		};
		_value = [_value] call DB_fnc_mresArray;
		_query = format ["UPDATE players SET experiencia='%1' WHERE pid='%2'",_value,_uid];
    };	
