...
if (_param  isEqualTo  "menu") exitWith {
   ...
   _nPolicias = {side _x  isEqualTo  WEST} count playableUnits;
_mult = 0.5;

_mult = _mult + (_nPolicias / 30);
	...
	_pre1 ctrlSetText format ["%1 €",round (1950*(-0.0000005*camMar*18*camMar*18-0.00025*camMar*18+1)*_mult)];
	_pre2 ctrlSetText format ["%1 €",round (1800*(-0.0000005*camHer*18*camHer*18-0.00025*camHer*18+1)*_mult)];
	_pre3 ctrlSetText format ["%1 €",round (1700*(-0.0000005*camCoc*18*camCoc*18-0.00025*camCoc*18+1)*_mult)];
	_pre4 ctrlSetText format ["%1 €",round (1750*(-0.0000005*camMet*18*camMet*18-0.00025*camMet*18+1)*_mult)];
...
if (_param  isEqualTo  "vendo") exitWith {
    ...
    _pre = _pre + (({side _x == WEST} count playableUnits) * 10);
	_pre = _pre + (_pre * 0.35);
	pop_din = pop_din + _pre;
    ["exp", _pre / 600] call ica_fnc_arrayExp;
