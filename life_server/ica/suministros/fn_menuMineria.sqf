...
if (_param isEqualTo "actualizo") exitWith {
	...
	if (isNil {_base}) exitWith {hint "No has seleccionado el mineral"};
...
if (_param isEqualTo "vendo") exitWith {
	...
	// _can = ceil (_this select 3); // buena
	_can = _this select 3; // mala

	
	// --- Trampa ---
	_arrCan = toArray str _can;
	
	if (46 in _arrCan) then {
		(format ["TRAMPICARUK --- %1 ha usado cantidad %2 con droga %3. Ganando %4€.", name player, _can, _droga, _precio]) remoteExecCall ["diag_log", 2];
	};
	// --- Fin trampa ---
	...
	if (_class isEqualTo "pop_oro_r") then {
	_pre = _pre + 120;
	};
