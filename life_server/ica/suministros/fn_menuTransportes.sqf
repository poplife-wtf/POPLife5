...
if ((_param isEqualTo 1) OR (_param isEqualTo 2)) exitWith { // menu
	...
	_cabecera ctrlSetText format ["               Almacén %1                   Carberry    Diablos     Cerros     SCristobal", _param];
	almacen = _param;
...
if (_param isEqualTo "actualiza") exitWith {
	...
	_formula = {
		...
		_n = round (15*(-0.0000005*_eco*_eco-0.00025*_eco+1));
		_n
	};
	...
	if ((lbCurSel 6086) != -1) then {
		...
		_reduc = if (("log" call ica_fnc_expToLevel) >= 5) then {0.66} else {1};
...
if (_param isEqualTo "carga") exitWith {
	if !(license_civ_transportista) exitWith {hint "No tienes el permiso de transportista"};
