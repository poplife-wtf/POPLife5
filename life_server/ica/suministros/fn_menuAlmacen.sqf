...
if (_param isEqualTo "menu") exitWith {
	...
	if (almacen == 1) then {
		var1 = alm1Ali; //alimento
		var2 = alm1Agu; //agua
		var3 = alm1Com; //combustible
		var4 = alm1Mad; //madera
	} else {
		var1 = alm2Ali;
		var2 = alm2Agu;
	...
	_pre1 ctrlSetText format ["%1 €", round(70*(-0.0000005*var1*var1-0.00025*var1+1))];
	_pre2 ctrlSetText format ["%1 €", round(50*(-0.0000005*var2*var2-0.00025*var2+1))];
	_pre3 ctrlSetText format ["%1 €", round(80*(-0.0000005*var3*var3-0.00025*var3+1))];
	_pre4 ctrlSetText format ["%1 €", round(50*(-0.0000005*var4*var4-0.00025*var4+1))];
...
if (_param isEqualTo "vendo") exitWith {
	...
	_aliexp = 0.08;
	if ((npc_presidente getVariable ["decreto",0]) == 2) then {_aliexp = 0.16};
		
	_expe = switch (_cosa) do {
	    case "Ali": {_aliexp};
        case "Agu": {0.06};
        case "Com": {0.15};
        case "Mad": {0.15};
	...
	["exp",_can * _expe] call ica_fnc_arrayExp;
			
	_organizacion = cursorTarget getVariable["controlado_por",grpNull];
	if(!isNull _organizacion) then 
	{ 
	[(round (_pre * 0.10)),_organizacion] call life_fnc_gangExtorsion;  
	};
