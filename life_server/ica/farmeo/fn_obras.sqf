...
if (_param isEqualTo "reparo") exitWith {
	...
	_i = 0;
	{
		if (!((_x iskindof "man") or (_x iskindof "car") or (_x iskindof "Motorcycle") or (_x iskindof "ship") or (_x iskindof "air") or (damage _x isEqualTo 0))) then {
			_x setdamage 0;
			_i = _i + 1;
		};
	} foreach _objetos;

	if (_i isEqualTo 0) exitwith {hint "Parece que aquí no había nada que reparar"};
	...
	if (_i > 0 && _i <= 3) exitwith {
		...
		_a setvariable ["persistent", true, true];
...
if (_param isEqualTo "descargo") exitWith {
	...
	{
		if ((_x getvariable "obrero") isEqualTo name player) then {
			deletevehicle _x;
		};
	} foreach _cargo;
	_exp = 3;
	_valor = 100*((count _cargo1) + (count _cargo2)*2 + (count _cargo3)*4);
	if (_valor isEqualTo 0) exitwith {hint"Ponte a currar cacho vago"};
