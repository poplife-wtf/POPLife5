...
switch (_param) do {
	...
	case "alimentos": {	
		_listaAlimentos = ["pop_rabano_i", "pop_girasol_i", "pop_maiz_i", "pop_trigo_i", "pop_pez","pop_pez2","pop_pez3","pop_pez4","pop_tiburon","pop_filete"];
		_puedo = false;
		_idx = 0;
		while {_idx < (count _listaAlimentos)} do {
			...
			if (([_param] call ica_fnc_tengo) >= 1) exitWith {
				_tengo = [_param] call ica_fnc_tengo;
				_tengo = if (_param  isEqualTo  "pop_tiburon" OR _param isEqualTo "pop_filete") then {_tengo * 5} else {_tengo};
				_puedo = true;
			};
			_idx = _idx + 1;
		};
