...
if ((typeName _param)  isEqualTo  "BOOL") exitWith {hint "Error"};
...
if (_param  isEqualTo  "planto") exitWith {
	...
	if ((_semilla isEqualTo "cannabis") AND (("ti" call ica_fnc_expToLevel) < 2)) exitWith {hint "No sabes plantar eso"};
	...
	_t = 3;
	_t = if (license_civ_agricultor) then {3} else {8};
	...
	_class = switch (_semilla) do {
		...
		case "algodon":	{["pop_algodon_plan", (60 * _t), "pop_algodon_s", ["TPHneutro1", "TPHneutro2", "TPHneutro3"]]};
	...
	if ((typeName _class)  isEqualTo  "BOOL") exitWith {hint "Error"};


	
	_granja = nearestObject [player, "House"]; 
	if (!isnil "_granja" && typeof _granja in listaGranjas && (_granja getVariable ["house_owner",[]] select 0) isEqualTo (GetPlayerUID player)) then {
	}else {
	...
	if (count _cercania  isEqualTo  0) then {_cercania = false} else {_cercania = true};
...
if (_param  isEqualTo  "recolecto") exitWith {
	...
	if ((((_cercania select 0) getVariable "crecimiento") < 100) OR (isNil {(_cercania select 0) getVariable "crecimiento"})) then {
		...
		_pillo = switch (_que) do {
			...
			case "pop_flower_roja": 	{"pop_flower_roja_inv" };
			case "pop_flower_verd":		{"pop_flower_verd_inv" };
			case "pop_flower_azul": 	{"pop_flower_azul_inv" };
			case "pop_flower_amar": 	{"pop_flower_amar_inv" };
			case "pop_flower_mora": 	{"pop_flower_mora_inv" };
			case "pop_flower_marr":		{"pop_flower_marr_inv" };
			case "pop_flower_negr": 	{"pop_flower_negr_inv" };
		};

		_n = if (license_civ_agricultor) then {4} else {2};
