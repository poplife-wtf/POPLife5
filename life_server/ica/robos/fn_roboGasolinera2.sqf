...
if (_param isEqualTo "init") exitWith {
	...
	if (_count < 6) exitWith {hint "Con eso me amenazas ? es de juguete estoy seguro ... tira chinches no ?"};	
	if (!isNil {paco getVariable "jodido"}) exitWith {hint "A Paco ya le han jodido hace poco"}; // existe la var, te esperas
	...
	if (isNil {paco getVariable "jodido"}) then { // no existe la var, la pongo
		paco setVariable ["jodido", true, true];
	};
	...
	titleText ["No dejes de apuntar a Paco para que no avise a la policía", "PLAIN", 0.5];
	player reveal paco;
	sleep 3;

	_t = 90;
	_tqAvisa = round (random [30,50,60]);	
	while {true} do {
		if (cursortarget != paco) exitWith {_cagada = true};
		if ((player distance paco) > 4) exitWith {_cagada = true};
    ...
    [paco, "alarmcar"] remoteExec ["life_fnc_say3D",0];
		sleep 2;
		_t = _t - 2;
		hintSilent format ["Paco necesita %1 segundos más para abrir la caja", _t];
	};

	if ((floor random 99) < 5) exitWith {
		player setDamage 1;
		[10, "Se le ha ido la cabeza...", [1,0,0], 1.5, format ["...Paco ha matado a %1 y se ha meado en su cara.", name player], [1,1,1], 1] remoteExec ["ica_fnc_anuncio",0];		
	...
	if (!isNil {_cagada}) then {
		...
		[] spawn {
			sleep 60;
			paco setVariable ["jodido", nil, true];
		};
	} else {
...
_pasta= _polisOn * 1000;
 pop_din = pop_din + 1500 + round random 1000 + _pasta;
		...
		paco setVariable ["jodido", name player, true];
	...
	//[[paco, ["PROTEGER ZONA - SEGURIDAD PRIVADA",{["protegerZonaSeguridadPrivada",paco] spawn ica_fnc_seguridadPrivada;}]],"addAction",true,true] call BIS_fnc_MP;
};

if (_param isEqualTo "pregunto") exitWith {
	_asd = paco getVariable "jodido";
	...
	hint format ["Los análisis han dado positivo: \n\nSe han encontrado fibras, sangre o fluídos de un tal %1.", paco getVariable "jodido"];
};
