...
if (_param isEqualTo "init") exitWith {
	_resetJoyeria = {
		...
		for "_x" from 1 to (count _cajas) do {
			(_cajas call BIS_fnc_selectRandom) addItemCargoGlobal ["pop_diamante", round (random 8)];
		};
	};
	sleep (60 * 20);
	call _resetJoyeria;
...
if (_param isEqualTo "robo") exitWith {
	if (sunOrMoon == 1) then {
		...
		//["roboDia"] spawn ica_fnc_roboJoyeria;
		["roboNoche"] spawn ica_fnc_roboJoyeria;
	};
...
if (_param isEqualTo "roboDia") exitWith {
	...
	if ((count units (group player)) < 6) exitWith {hint "No tienes suficientes miembros en tu organización ahora mismo como para intentar atracar la joyeria"};
...
if (_param isEqualTo "roboNoche") exitWith {
	...
	["menu", 0.98, "joyeria"] spawn ica_fnc_ganzua;
