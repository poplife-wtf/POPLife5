...
if (_param == "menu") exitWith {
	if (playerSide == WEST)exitWith{hint "Policia corrupto?";};
	...
	if ((count units (group player)) < 5) exitWith {hint "No tienes suficientes miembros en tu organización ahora mismo como para intentar robar la casa del alcalde"};
...
if (_param == "DSC") exitWith {
	...
	if ((estadoPortatil select 0) == 0) then { // fase 0
		...
		if ((count units (group player)) < 7) exitWith {hint "No tienes suficientes miembros en tu organización ahora mismo como para robar casa de alcalde"};
		...
		closeDialog 0;
		["loop"] remoteExec ["ica_fnc_roboAlcalde",2];
...
if (_param == "cajafuerte") exitWith {
	...
	["cajaloop"] remoteExec ["ica_fnc_roboAlcalde",2];
};
...
if (_param == "roboDinero") exitWith {
	...
	if (!isNil {dineroalcalde}) exitWith {hint "Ya se ha retirado el dinero"};
	...
	["reset"] remoteExec ["ica_fnc_roboAlcalde", 2];
	...
	_aumento = (_polisOn * 6500);

	_robado = _din + _aumento;
	pop_din = pop_din + _robado;
	hint format ["Has robado %1€", _robado];

	
	["envio", player, 50, 60*30] call ica_fnc_bloqueoDinero;
	ocupado = nil;
	//[[getPlayerUID player,profileName,"487"],"life_fnc_wantedAdd",(group player),false] call life_fnc_MP;
...
if (_param isEqualTo "useATM") exitWith {
	if ((player distance Portatilalcade) < 160) then {
