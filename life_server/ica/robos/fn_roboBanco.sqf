...
if (_param isEqualTo "init") exitWith { //sólo server
	_fnc = {
		...
		formula set [1, 1 + (round random 3)];
		formula set [2, 1 + (round random 4)];
		formula set [3, 0 + (round random 3)];
	};
...
if (_param isEqualTo "abrepuerta") exitWith {
	...
	if ((count units (group player)) < 7) exitWith {hint "No tienes suficientes miembros en tu organización ahora mismo como para intentar robar la casa del alcalde"};
...
if (_param isEqualTo "abrecaja") exitWith {
	if !(dialog) then {
		...
		[player, ["codigoBanco", "formula"]] spawn ica_fnc_varToServer; waitUntil {recibido};
		sleep 1;
		_code = ["encripta", codigoBanco] call ica_fnc_roboBanco;
		if (isNil {_code}) exitWith {hint "error: code nil"};
		if ((typeName _code) != "STRING") exitWith {hint "error: code !str"};
		if ((count _code) < (count codigoBanco)) exitWith {};
		
		
		if ((_this select 1) != _code) exitWith {hint "Acceso denegado"};
		if (!dialog) exitWith {};
		closeDialog 0;
