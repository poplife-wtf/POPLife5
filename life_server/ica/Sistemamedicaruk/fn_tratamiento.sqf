...
switch (_cosa) do {
	case "pop_vendas": {
		...
		if (vehicle player == player) then {
			if (_lvlmed <= 1) then {
			["medi3d", "bkSndFxMedicoVendar", _victima] call ica_fnc_SFX;
			[6, "Aplicando vendaje..."] spawn ica_fnc_barrita;
			player playmove "AinvPknlMstpSnonWrflDr_medic2";
			sleep 6;
			player switchMove "";} else {
			["medi3d", "bkSndFxMedicoVendarRap", _victima] call ica_fnc_SFX;
			[2, "Aplicando vendaje rapidamente..."] spawn ica_fnc_barrita;
			player playmove "AinvPknlMstpSnonWrflDr_medic2";
			sleep 2;
			};
		} else {
	...
	case "pop_vendas_c": {
		...
		if (vehicle player == player) then {
			if (_lvlmed <= 1) then {
			["medi3d", "bkSndFxMedicoVendar", _victima] call ica_fnc_SFX;
			[7, "Aplicando vendaje compresivo..."] spawn ica_fnc_barrita;
			player playmove "AinvPknlMstpSnonWrflDr_medic5";
			sleep 7;		
			player switchMove "";
			} else {
			["medi3d", "bkSndFxMedicoVendarRap", _victima] call ica_fnc_SFX;
			[4, "Aplicando vendaje compresivo rapidamente..."] spawn ica_fnc_barrita;
			player playmove "AinvPknlMstpSnonWrflDr_medic5";
			sleep 3;		
			player switchMove "";
			};
		...
		["medi3d", "bkSndFxMedicoVendarRap", _victima] call ica_fnc_SFX;
		[1, "Aplicando vendaje compresivo..."] spawn ica_fnc_barrita;
		sleep 1;
		};
	...
	case "pop_tramadol": {
		...
		if (vehicle player == player) then {
			["medi3d", "bkSndFxMedicoInyectar", _victima] call ica_fnc_SFX;
			[8, "Suministrando tramadol..."] spawn ica_fnc_barrita;
			player playmove "AinvPknlMstpSnonWrflDr_medic1";
			sleep 8;	
			player switchMove "";		
		} else {
		["medi3d", "seleccionarTramadol", _victima] call ica_fnc_SFX;
	...
	case "pop_morfina": {
		...
		["medi3d", "seleccionarMorfina", _victima] call ica_fnc_SFX;
	...
	case "pop_epinefrina": {
		...
		if (_zona == "pecho") exitWith {hint "La epinefrina no puede aplicarse en el pecho."; ocupado = nil};
		if (player == _victima) exitWith {hint "No puedes aplicarte ese tratamiento a ti mismo"; ocupado = nil};
		[_cosa, -1] call ica_fnc_item;
		[format ["-1 %1", [_cosa] call ica_fnc_classToname]] call ica_fnc_infolog;
		...
		if (vehicle player == player) then {
			...
			_victima setVariable ["reanimado",true,true];
	...
	case "pop_desfibrilador": {
		...
		["medi3d", "bkSndFxMedicoDesfibri2", _victima] call ica_fnc_SFX;
	...
	case "pop_bolsadesangre": {
		...
		["medi3d", "seleccionarSangre", _victima] call ica_fnc_SFX;
	...
	case "pop_csi": {
		if (_lvlmed < 5) exitWith {hint "No tienes los conocimientos para hacer eso"; ocupado = nil};
		if (!detective) exitWith {hint "No eres un detective policial"; ocupado = nil};
		...
		["medi3d", "seleccionarKitForense", _victima] call ica_fnc_SFX;
		[8, "Investigando..."] spawn ica_fnc_barrita;
		player playMove "AinvPknlMstpSlayWrflDnon_medic";
		sleep 8;
		player switchMove "";

		_arr = _victima getVariable ["culpables",["nada","nada"]];
		_str = "";
