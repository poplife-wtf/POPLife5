...
if (_param isEqualTo "recibo") exitWith {
	...
	//If ( _de == player or _de == name player ) exitWith {};
...
if (_param == "envio") exitWith {
	...
	_noLoencuentro = false;
	If ( movilIsReply ) Then {
		_noLoencuentro = true;
		{
			If ( name _x == _quien ) exitWith {
				_quien = _x ; 
				_noLoencuentro = false;
				//hint str _quien 
			}; 
	...
	if (_noLoencuentro) exitWith { ["play", "MSG_SndMsgNotify3", player] call ica_fnc_SFX ; hint "No se ha encontrado el destinatario..."};
	...
	if ( texto == "" ) exitWith { ["play", "MSG_SndMsgNotify3", player] call ica_fnc_SFX ; hint "La idea es que escribas algo..."};
