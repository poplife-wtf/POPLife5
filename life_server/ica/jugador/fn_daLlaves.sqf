...
if (_param isEqualTo "menu") exitWith {
	...
	_lista ctrlSetEventHandler ["LBSelChanged", "
		...
		_boton buttonSetAction ""
			['doy', lbData [7075, (lbCurSel 7075)], lbData [7076, (lbCurSel 7076)]] spawn ica_fnc_daLlaves;
		"";
	"];
...
if (_param isEqualTo "doy") exitWith {
	...
	hint format["Has dado las llaves de %2",_quien getVariable["realname",name _quien], getText(configFile >> "CfgVehicles" >> (typeOf _llave) >> "displayName")];
	//[[_llave,_quien,profileName], "TON_fnc_clientGetKey",_quien,false] call life_fnc_MP;
	//[_llave,_quien,profileName] remoteExec ["TON_fnc_clientGetKey",_quien];
	[_llave,_quien,profileName] remoteExec ["TON_fnc_clientGetKey",_quien];
};
