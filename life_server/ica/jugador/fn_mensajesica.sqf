...
if (_param isEqualTo "envio") exitWith {
	...
	if (_modo isEqualTo "taxi") exitWith {
		0 spawn ica_fnc_taxi_llamada;
	};
	if (_modo isEqualTo "seguridad privada") exitWith {
		0 spawn ica_fnc_sp_llamada;
	};	
	if (isNil {_leoMensajeDeApagado}) then {_leoMensajeDeApagado = false};
	
	if (typeName _quien == "BOOL") exitWith {
		if ((["pop_movil_s"] call ica_fnc_tengo) != 0) then {
		[["reciboa", player, texto, true, _leoMensajeDeApagado], "ica_fnc_mensajesica", true, false, false] call life_fnc_MP;
		} else {
	...
	if ((["pop_movil_s"] call ica_fnc_tengo) != 0) then {
		[["reciboa", player, texto, false, _leoMensajeDeApagado], "ica_fnc_mensajesica", _quien, false, false] call life_fnc_MP;
		} else {
...
if (_param isEqualTo "reciboa") exitWith {
	...
	if (
		((["pop_movil_s"] call ica_fnc_tengo) == 0) AND
	...
	hint parsetext format ["<t size='1.5' color='#F7FE2E'> Mensaje Anonimo <br/><br/></t>%1 ", _texto];
...
if (_param isEqualTo "recibo") exitWith {
	...
	_de = "";
	_remitente = _this select 1;
	if (typeName _remitente == "OBJECT") then {
	_de = name (_remitente);
	} else {
	_de = "..Oculto..";
	};
