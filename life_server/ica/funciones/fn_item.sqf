...
Se invoca con
	["classname"] call des_fnc_item				// te da el objeto 1 vez y te devuelve true mientras te quepa
	["classname", cantidad] call des_fnc_item	// te da el objeto X veces y te devuelve true mientras te quepa
	["classname", -cantidad] call des_fnc_item	// elimina el objeto X veces
	"classname" call des_fnc_item 				// te devuelve el tipo de objeto
*/
/*
...
if (_this isEqualType []) then {
	...
	_cantidad = abs _cantidad;
};
if (_this isEqualType "") then {
	_class = _this;
	_modo = true; // para que no de error
	_cantidad = ""; // lo convierto en string para que en la lin95 me devuelva el tipo de objeto
};

if !(_modo isEqualType true) exitWith {}; // ¿qué haces?
	...
	if (_modo) then { // true es añadir, false es quitar, obviamente.
		...
		for "_x" from 1 to _cantidad do {
			switch (toLower _queEs) do {
				...
				case "gaf": {
					if ((goggles player) == _class) then {removeGoggles player};
				};
			};
